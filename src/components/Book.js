...
export default function Book(props) {
  ...
  function statusHandler() {
    const bookId = props.book._id
    const newBook = {...props.book, status: props.book.status === "read" ? "unread" : "read"}
    ...
    axios.post(`http://localhost:5000/books/update/${bookId}`, newBook)
  ...
  return (
    <div className={`book${props.book.status === "read" ? " book-read" : ""}`}>
      ...
      <div className="status-div">
        <p style={{color: props.book.status === "unread" ? 'red' : 'green'}} className="read-status">
